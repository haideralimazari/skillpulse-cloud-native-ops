...
DOCKERHUB_USERNAME ?= $(shell grep DOCKERHUB_USERNAME .env 2>/dev/null | cut -d= -f2 || echo haideralimazari)
BACKEND_IMAGE  ?= $(DOCKERHUB_USERNAME)/skillpulse-backend:latest
FRONTEND_IMAGE ?= $(DOCKERHUB_USERNAME)/skillpulse-frontend:latest
